...
class BaseTimeSeriesCrossValidator:
    ...
    def __init__(self, n_splits=10):
        """
        :param n_splits (optional): Number of folds. Must be at least 2. Defaults to 10.
        """
        if not isinstance(n_splits, numbers.Integral):
            raise ValueError(
                f"The number of folds must be of Integral type. {n_splits} of type {type(n_splits)}" f" was passed."
            )
...
class PurgedWalkForwardCV(BaseTimeSeriesCrossValidator):
    ...
    def __init__(self, n_splits=10, n_test_splits=1, min_train_splits=2, max_train_splits=None):
        """
        :param n_splits (optional): Number of folds. Must be at least 2.. Defaults to 10.
        :param n_test_splits (optional): Number of folds used in the test set. Must be at least 1. Defaults to 1.
        :param min_train_splits (optional):  Minimal number of folds to be used in the train set. Defaults to 2.
        :param max_train_splits (optional): Maximal number of folds to be used in the train set.
                                            If None, there is no upper limit. Defaults to None.
        """
    ...
    def split(
        ...
        not labels.
        :param X: Samples. Only used to extract n_samples
        :param y (optional): not used, inherited from _BaseKFold. Defaults to None.
        :param pred_times (optional): Times at which predictions are made. pred_times.index has to coincide with
                                      X.index. Defaults to None.
        :param eval_times (optional): imes at which the response becomes available and the error can be computed.
                                      eval_times.index has to coincide with X.index.. Defaults to None.
        :param split_by_time (optional): If False, the folds contain an (approximately) equal number of samples.
                                         If True, the folds span time intervals of identical length. Defaults to False.

        Yields:
        train_indices: A numpy array containing all the indices in the train set.
        test_indices : A numpy array containing all the indices in the test set.
        """
    ...
    def compute_train_set(self, fold_bound: int, count_folds: int) -> np.ndarray:
        ...
        :param fold_bound: Bound between the train set and the test set.
        :param count_folds: The number (starting at 0) of the first fold in the test set.
        :return: A numpy array containing all the indices in the train set.
        """
    ...
    def compute_test_set(self, fold_bound: int, count_folds: int) -> np.ndarray:
        ...
        :param fold_bound: Bound between the train set and the test set.
        :param count_folds: The number (starting at 0) of the first fold in the test set.
        :return: A numpy array containing the test indices.
        """
        if self.n_splits - count_folds > self.n_test_splits:
...
class CombPurgedKFoldCV(BaseTimeSeriesCrossValidator):
    ...
    def __init__(self, n_splits=10, n_test_splits=2, embargo_td=pd.Timedelta(minutes=0)):
        """
        :param n_splits (optional): Number of folds. Must be at least 2.. Defaults to 10.
        :param n_test_splits (optional): Number of folds used in the test set. Must be at least 1. Defaults to 2.
        :param embargo_td (optional): Embargo period (see explanations above). Defaults to pd.Timedelta(minutes=0).
        """
        super().__init__(n_splits)
        if not isinstance(n_test_splits, numbers.Integral):
            raise ValueError(
    ...
    def split(
        ...
        :param pred_times (optional):  Times at which predictions are made. pred_times.index has to coincide with
                                       X.index. Defaults to None.
        :param eval_times (optional): Times at which the response becomes available and the error can be computed.
                                      eval_times.index has to coincide with X.index.. Defaults to None.
        :param split_by_time (optional): If False, the folds contain an (approximately) equal number of samples.
                                         If True, the folds span time intervals of identical length. Defaults to False.
        Yields:
        ...
        if split_by_time:
            ...
            dates = pred_times.unique()
            dates.sort()
            dates_splits = np.array_split(dates, self.n_splits)
            fold_indices = [data[np.isin(pred_times, dates_splits[i])].index for i in range(len(dates_splits))]
            fold_bounds = [(fold[0], fold[-1] + 1) for fold in [(fold[0], fold[-1] + 1) for fold in fold_indices]]
        else:
    ...
    def compute_train_set(self, test_fold_bounds: List[Tuple[int, int]], test_indices: np.ndarray) -> np.ndarray:
        """
        Compute the position indices of samples in the train set.
        :param test_fold_bounds: Each tuple records the bounds of a block of indices in the test set.
        :param test_indices: A numpy array containing all the indices in the test set.
        :return: A numpy array containing all the indices in the train set.
        """
    ...
    def compute_test_set(self, fold_bound_list: List[Tuple[int, int]]) -> Tuple[List[Tuple[int, int]], np.ndarray]:
        """
        Compute the indices of the samples in the test set.
        :param fold_bound_list: Each tuple records the bounds of the folds belonging to the test set.
        :return
            test_fold_bounds: Like fold_bound_list, but with the neighboring folds in the test set merged.
            test_indices: A numpy array containing the test indices.
        """
...
def compute_fold_bounds(cv: BaseTimeSeriesCrossValidator, split_by_time: bool) -> List[int]:
    ...
    :param cv: Cross-validation object for which the bounds need to be computed.
    :param split_by_time: If False, the folds contain an (approximately) equal number of samples.
                          If True, the folds span identical time intervals.
    :return:  A list of fold bounds
    """
    if split_by_time:
...
def embargo(
    cv: BaseTimeSeriesCrossValidator, train_indices: np.ndarray, test_indices: np.ndarray, test_fold_end: int
) -> np.ndarray:
    """
    ...
    :param cv: Needs to have the attributes cv.pred_times, cv.eval_times, cv.embargo_dt and cv.indices.
    :param train_indices: A numpy array containing all the indices of the samples currently included in the train set.
    :param test_indices: A numpy array containing all the indices of the samples in the test set.
    :param test_fold_end: Index corresponding to the end of a test set block.
    :return: The same array, with the indices subject to embargo removed.
    """
    if not hasattr(cv, "embargo_td"):
        raise ValueError(
            "The passed cross-validation object should have a member cv.embargo_td defining the embargo" "time."
        )
...
def purge(
    cv: BaseTimeSeriesCrossValidator, train_indices: np.ndarray, test_fold_start: int, test_fold_end: int
) -> np.ndarray:
    """
    ...
    :param cv: Needs to have the attributes cv.pred_times, cv.eval_times and cv.indices.
    :param train_indices: A numpy array containing all the indices of the samples currently included in the train set.
    :param test_fold_start: Index corresponding to the start of a test set block.
    :param test_fold_end: Index corresponding to the end of the same test set block.
    :return: A numpy array containing the train indices purged at test_fold_start.
    """
    ...
    # Purge the start of the train set that is after the test fold
    if test_fold_end < cv.indices.shape[0]:
        eval_time_test_end = cv.eval_times.iloc[test_fold_end]
        train_indices_2 = np.intersect1d(train_indices_2, cv.indices[cv.pred_times >= eval_time_test_end])
